...
namespace LeedsSharp.ElasticSearch.Common.Services
{
	...
	{
        public static IElasticClient EsClient => SearchClientHelper.GetClient();
		...
		{
			await EsClient.IndexAsync(item, d => d.Index("user"));
		}
		...
		public async Task AddItemsToIndexAsync(IEnumerable<User> items)
		{
			if (items.Count() == 1)
			{
				await EsClient.IndexAsync(items.First(), d => d.Index("user"));
			}
			else
			{
				Parallel.ForEach(items, async (x, y) =>
				{
					await EsClient.IndexAsync(x, d => d.Index("user"));
				});
			}
		}
		...
		public async Task UpdateItemsInIndexAsync(IEnumerable<User> items)
		{
			...
			{
				var response = await EsClient.UpdateAsync(DocumentPath<User>.Id(item.Id), x => x.Index("user").Doc(item));
				if (!response.IsValid)
				{
		...
		public static async Task<bool> IndexExistsAsync()
		{
			try
			{
				var response = await EsClient.IndexExistsAsync("user");
		...
		public static async Task DeleteIndexAsync()
		{
			try
			{
				var response = await EsClient.DeleteIndexAsync("user");
			}
			catch (Exception ex)
			{
